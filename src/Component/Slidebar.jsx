  ...
  return (
    <aside className="h-full w-20 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200">
      ...
      <div className="overflow-y-auto w-full py-3">
        <div className="flex flex-col gap-1 max-h-[65%] max-sm:max-h-[75%] overflow-y-scroll no-scrollbar">
        ...
        <dialog id="my_modal_3" className="modal">
          <div className="modal-box">
            ...
            />
            <div className="flex flex-col gap-1 h-60 overflow-y-scroll no-scrollbar mt-3">
              {filteredUsers.map((user) => (
                <button
                  key={user._id}
                  onClick={() => setGroupUsers((prev)=>prev.includes(user._id)?prev.filter((userId)=>userId!==user._id):[...prev,user._id])}
                  className={`
              w-full p-3 flex items-center gap-3
              hover:bg-base-300 hover:rounded-box transition-colors
              ${
